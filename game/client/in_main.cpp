...
void IN_SwitchDown(const CCommand &args) { KeyDown(&in_switch, args[1]); }
void IN_SwitchUp(const CCommand &args) { KeyUp(&in_switch, args[1]); }
...
int CInput::GetButtonBits( int bResetState )
{
	...
	CalcButtonBits(bits, IN_SWITCH, s_ClearInputState, &in_switch, bResetState);
...
static ConCommand startswitch("+switch", IN_SwitchDown);
static ConCommand endswitch("-switch", IN_SwitchUp);
